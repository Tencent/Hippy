...
jobject GetHeapCodeStatistics(__unused JNIEnv* j_env,
  ...
  auto runtime_id = static_cast<int32_t>(j_runtime_id);
  std::shared_ptr<Runtime> runtime = Runtime::Find(runtime_id);
  v8::HeapCodeStatistics* heap_code_statistics = new v8::HeapCodeStatistics();
  if (runtime) {
      v8::Isolate* isolate = std::static_pointer_cast<hippy::napi::V8VM>(runtime->GetEngine()->GetVM())->isolate_;
      isolate->GetHeapCodeAndMetadataStatistics(heap_code_statistics);
  } else {
      TDF_BASE_DLOG(WARNING) << "GetHeapCodeStatistics, j_runtime_id invalid";
  }
  // set HeapCodeStatistics data
  jclass java_class_hcs = j_env->FindClass("com/tencent/mtt/hippy/bridge/heap/HeapCodeStatistics");
  jmethodID hcs_constructor = j_env->GetMethodID(java_class_hcs,"<init>","(JJJ)V");
  jobject hcs_obj = j_env->NewObject(java_class_hcs,
                                     hcs_constructor,
  ...
  delete heap_code_statistics;
  heap_code_statistics = nullptr;
...
jobject GetHeapSpaceStatisticsList(__unused JNIEnv* j_env,
    ...
    // init ArrayList
    jclass java_class_list = j_env->FindClass("java/util/ArrayList");
    jmethodID list_constructor = j_env->GetMethodID(java_class_list , "<init>","()V");
    jobject list_obj = j_env->NewObject(java_class_list, list_constructor);
    jmethodID list_add  = j_env->GetMethodID(java_class_list,"add","(Ljava/lang/Object;)Z");

    auto runtime_id = static_cast<int32_t>(j_runtime_id);
    std::shared_ptr<Runtime> runtime = Runtime::Find(runtime_id);

    if (runtime) {
        jclass java_class_hss = j_env->FindClass("com/tencent/mtt/hippy/bridge/heap/HeapSpaceStatistics");
        jmethodID hss_constructor = j_env->GetMethodID(java_class_hss,"<init>","(Ljava/lang/String;JJJJ)V");
        v8::Isolate* isolate = std::static_pointer_cast<hippy::napi::V8VM>(runtime->GetEngine()->GetVM())->isolate_;
        ...
        for(size_t i = 0 ; i < space_count ; i++)
        {
            ...
            v8::HeapSpaceStatistics* heap_space_statistics = new v8::HeapSpaceStatistics;
            isolate->GetHeapSpaceStatistics(heap_space_statistics, i);
            jobject hss_obj = j_env->NewObject(java_class_hss,
                                               hss_constructor,
                                               j_env->NewStringUTF(heap_space_statistics->space_name()),
            ...
            j_env->CallBooleanMethod(list_obj, list_add, hss_obj);
            delete heap_space_statistics;
            heap_space_statistics = nullptr;
        }
    } else {
...
jobject GetHeapStatistics(__unused JNIEnv* j_env,
    ...
    v8::HeapStatistics* heap_statistics = new v8::HeapStatistics();
    auto runtime_id = static_cast<int32_t>(j_runtime_id);
    std::shared_ptr<Runtime> runtime = Runtime::Find(runtime_id);
    if (runtime) {
        v8::Isolate* isolate = std::static_pointer_cast<hippy::napi::V8VM>(runtime->GetEngine()->GetVM())->isolate_;
    ...
    jclass java_class_hs = j_env->FindClass("com/tencent/mtt/hippy/bridge/heap/HeapStatistics");
    jmethodID hs_constructor = j_env->GetMethodID(java_class_hs,"<init>","(JJJJJJJJJJJJJ)V");
    jobject hs_obj = j_env->NewObject(java_class_hs,
                                      hs_constructor,
    ...
    delete heap_statistics;
    heap_statistics = nullptr;
...
jint WriteHeapSnapshot(__unused JNIEnv* j_env,
    ...
    if (heap_snapshot_stream.save_res_ != HeapSnapshotOutputStreamAdapter::sOk) {
        ...
        return WRITE_HEAP_ERR_SAVE;
    }
    TDF_BASE_DLOG(INFO) << "WriteHeapSnapshot end";
    return WRITE_HEAP_OK;
}
