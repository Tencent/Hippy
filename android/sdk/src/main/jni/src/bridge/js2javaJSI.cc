...
unordered_map<std::string, jobject> objectList;
//unordered_map<std::string, jobject> classList;
...
void TurboModule::install(
    jsi::Runtime &runtime,
    const std::shared_ptr<TurboModule> turboModule,const std::string moduleName_)
{
  auto moduleName = moduleName_;
  auto object = jsi::Object::createFromHostObject(runtime, turboModule);
...
void CallJava(hippy::napi::CBDataTuple* data) {
  ...
  jobject j_object=objectList.find(j_module_name);

  j_env->CallVoidMethod(j_object, j_method, j_module_name,
