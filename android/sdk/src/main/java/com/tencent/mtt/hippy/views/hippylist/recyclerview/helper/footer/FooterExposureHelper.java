...
public class FooterExposureHelper extends RecyclerView.OnScrollListener implements OnAttachStateChangeListener {
    ...
    /**
     * 设置仅仅关心滚动事件产生的footer曝光事件，过滤那种recyclerView首次排版，数据量很少，直接显示了footer的情况
     */
    public void setCareScrollOnly(boolean careScrollOnly) {
        this.careScrollOnly = careScrollOnly;
    }
    ...
    private class CheckVisibleRunnable implements Runnable {
        ...
        CheckVisibleRunnable(boolean isScrolling) {
            this.isScrolling = isScrolling;
        }

        @Override
        public void run() {
            if (shouldNotify() && isViewChangeToVisible(exposureView)) {
                notifyFooterAppeared();
            }
        }

        private boolean shouldNotify() {
            return !careScrollOnly || (careScrollOnly && isScrolling);
        }
    }
    ...
    public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
        ...
        if (exposureView != null) {
            ...
            checkVisibleRunnable = new CheckVisibleRunnable(dx != 0 || dy != 0);
