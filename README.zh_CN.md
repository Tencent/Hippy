...
> 如果步骤4出现错误，可以先 `cd` 到 `framework/js/examples` hippy-react-demo 或者 hippy-vue-demo 目录下，执行 `npm install --legacy-peer-deps`，提前将 demo 的 NPM 包依赖先安装好。
>
...
2. `cd` 到 `framework/js/examples` hippy-react-demo 或者 hippy-vue-demo 目录。
