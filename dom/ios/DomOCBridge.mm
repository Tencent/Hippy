...
@implementation NSObject (DomOCBridge)

- (hippy::DomArgument)toDomArgument {
    return DomArgument([self toHippyValue]);
}

@end

CGRect CGRectMakeFromLayoutResult(LayoutResult result) {
    return CGRectMake(result.left, result.top, result.width, result.height);
}

UIEdgeInsets UIEdgeInsetsFromLayoutResult(LayoutResult result) {
...
NSDictionary *stylesFromDomNode(const std::shared_ptr<hippy::DomNode> &domNode) {
    ...
    if (styles) {
      NSDictionary *dicStyles  = UnorderedMapHippyValueToDictionary(styles);
      [allStyles addEntriesFromDictionary:dicStyles];
    }
    if (extStyles) {
      NSDictionary *dicExtStyles = UnorderedMapHippyValueToDictionary(extStyles);
