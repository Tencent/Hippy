<template>
  <div id="demo-swiper">
    ...
    <!--
      ...
      @param {Number} pageSize 指定每个page相对于屏幕宽度的百分比（取值范围：0.0~1.0）
      @param {Number} middlePageOffset 指定每个page相对于屏幕左边缘的偏移量。不设置该属性时则page居中显示。
...
export default {
  ...
  methods: {
    ...
    onDragging(evt) {
      /*
      iOS端向后翻页时，nextSlide先+1，然后offset从0递增到1；向前翻页时，nextSlide先-1，然后offset从0递减到-1。
      Android端的滚动事件与iOS端不同：
      向后翻页时nextSlide先不变，而是offset递增，在offset增加的1.0时进位，此时nextSlide+1，offset变为0，翻页完成。
      向前翻页时，offset先向nextSlide借位变为1.0，此时nextSlide-1，随着滚动offset递减，在offset递减到0.0时，翻页完成。
      */
