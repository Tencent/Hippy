...
export default {
  components: {
     ...
     *  import 出错时需在catch里做对应的降级方案
     */
    AsyncComponentFromHttp: process.env.NODE_ENV === 'development' ? () => import(/* webpackMode: "lazy",customChunkPath: "http://127.0.0.1:38989/", webpackChunkName: "asyncComponentFromHttp" */'./dynamicImport/async-component-http.vue').then(res => res).catch(err => console.error('import async remote component error', err)) : () => import(/* webpackMode: "lazy",customChunkPath: "https://static.res.qq.com/hippy/hippyVueDemo/path/", webpackChunkName: "asyncComponentFromHttp" */'./dynamicImport/async-component-http.vue').then(res => res).catch(err => console.error('import async remote component error', err)),
  },
  data() {
    return {
      loaded: false,
