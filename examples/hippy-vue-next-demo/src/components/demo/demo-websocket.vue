<template>
  <div id="websocket-demo">
    <div>
      ...
      <input ref="wsUrlRef" value="wss://echo.websocket.org" />
      <div class="row">
    ...
    <div>
      ...
      <input ref="messageRef" value="Rock it with Hippy WebSocket" />
    ...
    <div>
      ...
      <div class="output fullscreen">
        <div>
          <p v-for="(line, index) in output" :key="index">
            {{ line }}
          </p>
        </div>
      </div>
...
<script lang="ts">
  ...
  const connect = (url: string) => {
    ...
    wsInstance.onmessage = (message) =>
      appendOutput(`[Received] ${message.data}`);
  };
