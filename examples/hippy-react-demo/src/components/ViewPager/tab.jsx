...
export default class Tab extends Component {
  ...
  getLeftOffset() {
    ...
    let leftResStart = this.layouts[0].slideWidth + (wrapperPaddingLeft - this.slidebarWidth / 2); // 最左侧（起始位置）
    let leftRes = leftResStart;
    for (let i = startPosition; i >= 1; i--) {
      leftRes += this.layouts[i].slideWidth;
    }
    if (offset === 0) {
      leftRes = leftResStart;
      for (let i = position; i >= 1; i--) {
        leftRes += this.layouts[i].slideWidth;
      }
    } else {
      leftRes += offset * this.layouts[offset > 0 ? position : startPosition].slideWidth; // 移动过程中需要根据比例和当前slideWidth计算滑块位置
    }
    return leftRes;
  }
