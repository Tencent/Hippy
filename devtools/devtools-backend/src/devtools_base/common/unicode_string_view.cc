...
using unicode_string_view = tdf::devtools::stringview::unicode_string_view;
...
std::size_t std::hash<unicode_string_view::u8string>::operator()(
  ...
  return std::_Hash_impl::hash(value.data(), value.length() * sizeof(unicode_string_view::char8_t_));
}
#endif

std::size_t std::hash<unicode_string_view>::operator()(const unicode_string_view& value) const noexcept {
...
unicode_string_view::unicode_string_view(const unicode_string_view& source) : encoding_(source.encoding_) {
  switch (encoding_) {
...
bool unicode_string_view::operator!=(const unicode_string_view& rhs) const noexcept { return !operator==(rhs); }
