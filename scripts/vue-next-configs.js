...
const hippyVuePackage = require('../packages/hippy-vue-next/package.json');
...
function banner(name, version) {
  ...
  return `/*!
 * ${name} v${version}
 * (Using Vue v${VueVersion}${name !== '@hippy/vue-next' ? andHippyVueString : ''})
 * Build at: ${new Date()}
 *
...
const aliases = {
  ...
  '@vue2': resolvePackage('hippy-vue-next'),
...
const builds = {
  '@hippy/vue': {
    entry: resolvePackage('hippy-vue-next', 'src/index.js'),
    dest: resolvePackage('hippy-vue-next', 'dist/index.js'),
    format: 'es',
    banner: banner('@hippy/vue-next', hippyVuePackage.version),
  },
