...
NSString *const NativeRenderCollectionElementKindSectionHeader = @"NativeRenderCollectionElementKindSectionHeader";
NSString *const NativeRenderCollectionElementKindSectionFooter = @"NativeRenderCollectionElementKindSectionFooter";

@interface NativeRenderCollectionViewWaterfallLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id<NativeRenderCollectionViewDelegateWaterfallLayout> delegate;
...
@implementation NativeRenderCollectionViewWaterfallLayout

/// How many items to be union into a single rectangle
static const NSInteger unionSize = 20;

static CGFloat NativeRenderFloorCGFloat(CGFloat value) {
...
- (void)setItemRenderDirection:(NativeRenderCollectionViewWaterfallLayoutItemRenderDirection)itemRenderDirection {
...
- (CGFloat)itemWidthInSectionAtIndex:(NSInteger)section {
    ...
    return NativeRenderFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
}
...
- (id<NativeRenderCollectionViewDelegateWaterfallLayout>)delegate {
    return (id<NativeRenderCollectionViewDelegateWaterfallLayout>)self.collectionView.delegate;
}
...
- (void)commonInit {
    ...
    _itemRenderDirection = NativeRenderCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout {
    ...
    NSAssert([self.delegate conformsToProtocol:@protocol(NativeRenderCollectionViewDelegateWaterfallLayout)],
        @"UICollectionView's delegate should conform to NativeRenderCollectionViewDelegateWaterfallLayout protocol");
...
- (void)createItemAttributesInSection:(NSInteger)section {
    ...
    CGFloat width = self.collectionView.bounds.size.width - sectionInset.left - sectionInset.right;
    NSInteger columnCount = [self columnCountForSection:section];
    CGFloat itemWidth = NativeRenderFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
    ...
    for (NSInteger idx = 0; idx < itemCount; idx++) {
        ...
        if (itemSize.height > 0 && itemSize.width > 0) {
            itemHeight = NativeRenderFloorCGFloat(itemSize.height * itemWidth / itemSize.width);
        }
    ...
    *top += headerInset.top;
    ...
    if (headerHeight > 0) {
        UICollectionViewLayoutAttributes *attributes =
            [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:NativeRenderCollectionElementKindSectionHeader
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:NativeRenderCollectionElementKindSectionHeader]) {
        attribute = self.headersAttribute[@(indexPath.section)];
    } else if ([kind isEqualToString:NativeRenderCollectionElementKindSectionFooter]) {
        ...
        if (CGRectIntersectsRect(rect, attr.frame)) {
            switch (attr.representedElementCategory) {
                case UICollectionElementCategorySupplementaryView:
                    if ([attr.representedElementKind isEqualToString:NativeRenderCollectionElementKindSectionHeader]) {
                        supplHeaderAttrDict[attr.indexPath] = attr;
                    } else if ([attr.representedElementKind isEqualToString:NativeRenderCollectionElementKindSectionFooter]) {
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
    ...
    switch (self.itemRenderDirection) {
        ...
        case NativeRenderCollectionViewWaterfallLayoutItemRenderDirectionRightToLeft:
