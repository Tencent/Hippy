...
@implementation NativeRenderConvert (Transform)
...
+ (CATransform3D)CATransform3DFromMatrix:(id)json {
    ...
    if (![json isKindOfClass:[NSArray class]]) {
        NativeRenderLogError(@"[%@], a CATransform3D. Expected array for transform matrix.", json);
        return transform;
    }
    if ([json count] != kMatrixArrayLength) {
        NativeRenderLogError(@"[%@], a CATransform3D. Expected 4x4 matrix array.", json);
        return transform;
    }
    for (NSUInteger i = 0; i < kMatrixArrayLength; i++) {
        ((CGFloat *)&transform)[i] = [NativeRenderConvert CGFloat:json[i]];
    }
    return transform;
}
...
+ (CATransform3D)CATransform3D:(id)json {
    ...
    if (![json isKindOfClass:[NSArray class]]) {
        NativeRenderLogError(@"[%@],a CATransform3D. Did you pass something other than an array?", json);
        return transform;
    }
    // legacy matrix support
    if ([(NSArray *)json count] == kMatrixArrayLength && [json[0] isKindOfClass:[NSNumber class]]) {
        NativeRenderLogWarn(
            @"[NativeRenderConvert CATransform3D:] has deprecated a matrix as input. Pass an array of configs (which can contain a matrix key) instead.");
    ...
    for (NSDictionary *transformConfig in (NSArray<NSDictionary *> *)json) {
        if (transformConfig.count != 1) {
            NativeRenderLogError(@"[%@], a CATransform3D. You must specify exactly one property per transform object.", json);
            return transform;
        }
        NSString *property = transformConfig.allKeys[0];
        id value = NativeRenderNilIfNull(transformConfig[property]);
        if ([property isEqualToString:@"matrix"]) {
            ...
            CGFloat translateX = [NativeRenderNilIfNull(array[0]) floatValue];
            CGFloat translateY = [NativeRenderNilIfNull(array[1]) floatValue];
            CGFloat translateZ = array.count > 2 ? [NativeRenderNilIfNull(array[2]) floatValue] : 0;
            ...
            NativeRenderLogError(@"Unsupported transform type for a CATransform3D: %@.", property);
        }
    }
    return transform;
}
