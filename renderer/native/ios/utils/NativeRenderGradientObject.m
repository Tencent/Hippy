...
@implementation NativeRenderGradientLocationParser
...
@implementation NativeRenderGradientObject

- (void)drawInContext:(CGContextRef)context canvasInfo:(CanvasInfo)canvasInfo {
    switch (self.gradientType) {
        case NativeRenderGradientTypeLinear:
            NativeRenderDrawLinearGradientInContext(self, context, canvasInfo);
            break;
        case NativeRenderGradientTypeRadial:
            NativeRenderDrawRadialGradientInContext(self, context, canvasInfo.size);
            break;
        default:
            break;
    }
...
static BOOL getDirectionFromString(NSString *string, NativeRenderGradientDirection *direction) {
    BOOL isDrawnByDirection = NO;
    if (direction) {
        if ([string isEqualToString:@"totop"]) {
            *direction = NativeRenderGradientDirectionTop;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"totopright"]) {
            *direction = NativeRenderGradientDirectionTopRight;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"toright"]) {
            *direction = NativeRenderGradientDirectionRight;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"tobottomright"]) {
            *direction = NativeRenderGradientDirectionBottomRight;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"tobottom"]) {
            *direction = NativeRenderGradientDirectionBottom;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"tobottomleft"]) {
            *direction = NativeRenderGradientDirectionBottomLeft;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"toleft"]) {
            *direction = NativeRenderGradientDirectionLeft;
            isDrawnByDirection = YES;
        }
        else if ([string isEqualToString:@"totopleft"]) {
            *direction = NativeRenderGradientDirectionTopLeft;
            isDrawnByDirection = YES;
        }
    }
...
static CGFloat degreeFromDirection(NativeRenderGradientDirection direction, CGSize size) {
    CGFloat degree = 0.0f;
    switch (direction) {
        case NativeRenderGradientDirectionTopRight:
            degree = atan(size.height / size.width);
            break;
        case NativeRenderGradientDirectionRight:
            degree = M_PI_2;
            break;
        case NativeRenderGradientDirectionBottomRight:
            degree = M_PI - atan(size.height / size.width);
            break;
        case NativeRenderGradientDirectionBottom:
            degree = M_PI;
            break;
        case NativeRenderGradientDirectionBottomLeft:
            degree = M_PI + atan(size.height / size.width);
            break;
        case NativeRenderGradientDirectionLeft:
            degree = M_PI + M_PI_2;
            break;
        case NativeRenderGradientDirectionTopLeft:
...
static LinearGradientPoints pointsFromDirection(NativeRenderGradientObject *object, CGSize size) {
...
//      };
        @try {
            ...
            NativeRenderGradientDirection direction;
            ...
            NativeRenderGradientLocationParser *locationParser = [[NativeRenderGradientLocationParser alloc] initWithLocationsCount:[colorStopList count]];
