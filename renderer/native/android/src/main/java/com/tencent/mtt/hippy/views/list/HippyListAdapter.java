...
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG," onBindContentView updateViewRecursive");
      ...
      try {
        toNode = nativeRenderer.getRenderManager().getRenderNode(mParentRecyclerView.getId())
            .getChildAt(position);
      } catch (Throwable e) {
...
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG, "toNode: " + toNode.toString());
      //LogUtils.d("HippyListView", "fromNode: " + contentHolder.mBindNode.toString());
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG,"fromNode: " + contentHolder.mBindNode.toString());
...
////				nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG, patchType.mPatch.toString());
//			}

      //step:2 delete unUseful views
      DiffUtils.deleteViews(nativeRenderer.getRenderManager().getControllerManager(), patchTypes);
      //LogUtils.d("HippyListView", " deleteViews  position: " + position);
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG, " deleteViews  position: " + position);
      //step:3 replace id
      DiffUtils.replaceIds(nativeRenderer.getRenderManager().getControllerManager(), patchTypes);
      //LogUtils.d("HippyListView", " replaceIds  position: " + position);
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG, " replaceIds  position: " + position);
      //step:4 create view is do not  reUse
      DiffUtils.createView(patchTypes);
      //LogUtils.d("HippyListView", " createView  position: " + position);
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG, " createView  position: " + position);
      //step:5 patch the dif result
      DiffUtils.doPatch(nativeRenderer.getRenderManager().getControllerManager(), patchTypes);
      //LogUtils.d("HippyListView", " doPatch  position: " + position);
//			nativeRenderer.getGlobalConfigs().getLogAdapter().log(TAG, " doPatch  position: " + position);
  ...
  private RecyclerViewBase.ViewHolder getScrapViewForPositionInner(int position, int type,
    ...
    if (nativeRenderer.getRenderManager().getRenderNode(mParentRecyclerView.getId()) == null
        ||
        nativeRenderer.getRenderManager().getRenderNode(mParentRecyclerView.getId()).getChildCount()
            <= position) {
      return null;
    }
