...
public class BackgroundDrawable extends BaseDrawable {
    ...
    public void setBounds(int left, int top, int right, int bottom) {
        ...
        mShadowRect = new RectF(left + mShadowRadius - mShadowOffsetX,
                top + mShadowRadius - mShadowOffsetY, right - mShadowRadius - mShadowOffsetX,
...
//			dotBorderWidth = mBorderRadiusArray[0];
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP && // 只在有需要绘制border的时候，才使用bitmap
            ...
            if (canvasWidth != mCanvasBitmap.getWidth() || canvasHeight != mCanvasBitmap
                    .getHeight()) {
                try {
                    mCanvasBitmap = generateBitmap(canvas.getWidth(), canvas.getHeight());
                    /** draw on temp canvas **/
    ...
    private void calculateStartEndPoint(PointF start, PointF end, int oppositeDegree) {
        ...
        if ((gradientAngle > 90 && gradientAngle < 180) || (gradientAngle > 270
                && gradientAngle < 360)) {
    ...
    private boolean initGradientPaint() {
        ...
        if (gradientAngle == Integer.MAX_VALUE || gradientColors == null
    ...
    private void updatePath() {
        ...
        mPathForBorderRadius.addRoundRect(mTempRectForBorderRadius,
