...
static NSRegularExpression *VoltronJSStackFrameRegex()
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (regexError) {
            VoltronLogError(@"Failed to build regex: %@", [regexError localizedDescription]);
        }
    });
    return _regex;
}

@implementation VoltronJSStackFrame
...
{
    return @{
             @"methodName": VoltronNullIfNil(self.methodName),
             @"file": VoltronNullIfNil(self.file),
...
{
    NSTextCheckingResult *match = [VoltronJSStackFrameRegex() firstMatchInString:line options:0 range:NSMakeRange(0, line.length)];
    if (!match) {
        return nil;
    }
...
+ (NSArray<VoltronJSStackFrame *> *)stackFramesWithLines:(NSString *)lines
{
    NSMutableArray *stack = [NSMutableArray new];
    for (NSString *line in [lines componentsSeparatedByString:@"\n"]) {
        VoltronJSStackFrame *frame = [self stackFrameWithLine:line];
        if (frame) {
...
+ (NSArray<VoltronJSStackFrame *> *)stackFramesWithDictionaries:(NSArray<NSDictionary *> *)dicts
{
    NSMutableArray *stack = [NSMutableArray new];
    for (NSDictionary *dict in dicts) {
        VoltronJSStackFrame *frame = [self stackFrameWithDictionary:dict];
        if (frame) {
            [stack addObject:frame];
        }
    }
