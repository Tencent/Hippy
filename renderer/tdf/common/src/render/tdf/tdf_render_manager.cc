...
#include "render/tdf/vdom/embedded_view_node.h"
#include "render/tdf/vdom/image_view_node.h"
#include "render/tdf/vdom/list_view_node.h"
#include "render/tdf/vdom/modal_view_node.h"
#include "render/tdf/vdom/refresh_wrapper_node.h"
#include "render/tdf/vdom/root_view_node.h"
#include "render/tdf/vdom/scroll_view_node.h"
#include "render/tdf/vdom/text_input_node.h"
#include "render/tdf/vdom/text_view_node.h"
#include "render/tdf/vdom/view_names.h"
#include "render/tdf/vdom/view_pager_node.h"
...
void TDFRenderManager::RegisterShell(uint32_t root_id, const std::shared_ptr<tdfcore::Shell>& shell) {
  auto render_context = TDF_MAKE_SHARED(tdfrender::TDFRenderContext,root_id, shell, GetDomManager(), GetUriDataGetter());
  render_context_repo_.emplace(root_id, render_context);
}

void TDFRenderManager::CreateRenderNode(std::weak_ptr<RootNode> root_node, std::vector<std::shared_ptr<hippy::dom::DomNode>>&& nodes) {
  ...
  auto render_context = render_context_repo_.find(static_cast<uint32_t>(root->GetId()))->second;
  auto shell = render_context->GetShell();

  std::promise<void> view_create;
  shell->GetUITaskRunner()->PostTask([=, &view_create] {
    TDF_RUNNER_CHECK_UI;
    ...
    for (auto const& node : nodes) {
      ...
      assert(node->GetId() == node->GetRenderInfo().id);
...
void TDFRenderManager::UpdateRenderNode(std::weak_ptr<RootNode> root_node, std::vector<std::shared_ptr<DomNode>>&& nodes) {
  auto root = root_node.lock();
  if (!root) {
    return;
  }
...
void TDFRenderManager::MoveRenderNode(std::weak_ptr<RootNode> root_node, std::vector<std::shared_ptr<DomNode>>&& nodes) {

}

void TDFRenderManager::DeleteRenderNode(std::weak_ptr<RootNode> root_node, std::vector<std::shared_ptr<DomNode>>&& nodes) {
  auto root = root_node.lock();
  if (!root) {
    return;
  }
...
void TDFRenderManager::UpdateLayout(std::weak_ptr<RootNode> root_node, const std::vector<std::shared_ptr<DomNode>>& nodes) {
  auto root = root_node.lock();
  if (!root) {
    return;
  }
...
void TDFRenderManager::MoveRenderNode(std::weak_ptr<RootNode> root_node, std::vector<int32_t>&& moved_ids, int32_t from_pid, int32_t to_pid) {
  // TODO 暂不支持
  assert(false);
}
...
void TDFRenderManager::AddEventListener(std::weak_ptr<RootNode> root_node, std::weak_ptr<DomNode> dom_node, const std::string& name) {
  auto root = root_node.lock();
  if (!root) {
    return;
  }
...
void TDFRenderManager::RemoveEventListener(std::weak_ptr<RootNode> root_node, std::weak_ptr<DomNode> dom_node, const std::string& name) {
...
void TDFRenderManager::RegisterNodeCreator(const std::string& view_name, const tdfrender::node_creator& creator) {
  node_creator_tables_.insert(std::make_pair(view_name, creator));
}

tdfrender::node_creator TDFRenderManager::GetNodeCreator(const std::string& view_name) {
