...
#include "core/tdfi/view/image_load_manager.h"
...
class Base64ImageLoader : public tdfcore::ImageLoader {
 public:
  ~Base64ImageLoader() override = default;
  Base64ImageLoader() = default;

  std::shared_ptr<tdfcore::Task> Load(const std::string &url, const tdfcore::ImageOptions &options,
                                      const tdfcore::ImageProgressCallback &progress_callback,
                                      const tdfcore::ImageLoadCallback &loader_callback) override;
