  ...
  try {
    ...
    return pages.flat().map(page => {
      ...
      return {
        ...
        devtoolsFrontendUrl: `localhost:8080/public/inspector.html?experiments=true&ws=${encodeURIComponent(ws)}`,
        devtoolsFrontendUrlCompat: `localhost:8080/public/inspector.html?experiments=true&ws=${encodeURIComponent(ws)}`,
...
const getAndroidTargets = ({host, port, wsPath, clientId}) => {
  ...
  return androidTargetManager.getTargets().map(({id: targetId, ws}) => {
    ...
    return {
      ...
      devtoolsFrontendUrl: `localhost:8080/public/inspector.html?experiments=true&ws=${encodeURIComponent(wsUrl)}`,
      devtoolsFrontendUrlCompat: `localhost:8080/public/inspector.html?experiments=true&ws=${encodeURIComponent(wsUrl)}`,
