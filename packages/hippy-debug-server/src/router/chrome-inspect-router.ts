...
const getIosTargets = async ({
  ...
  try {
    ...
    return pages.flat().map(page => {
      ...
      const ws = `${host}:${port}${
        wsPath
      }?platform=${DevicePlatform.IOS}&clientId=${clientId}&targetId=${targetId}&role=${ClientRole.Devtools}&appClientType=${encodeURIComponent(JSON.stringify(appClientType))}&debugPage=${encodeURIComponent(
        JSON.stringify(page),
      )}`;
      ...
      return {
        ...
        devtoolsFrontendUrl: `http://localhost:${port}/front_end/inspector.html?remoteFrontend=true&experiments=true&ws=${encodeURIComponent(ws)}`,
        devtoolsFrontendUrlCompat: `http://localhost:${port}/front_end/inspector.html?remoteFrontend=true&experiments=true&ws=${encodeURIComponent(ws)}`,
...
const getAndroidTargets = ({host, port, wsPath, clientId}) => {
  ...
  return androidTargetManager.getTargets().map(({id: targetId, ws}) => {
    const wsUrl = `${host}:${port}${wsPath}?platform=${DevicePlatform.Android}&clientId=${clientId}&targetId=${targetId}&role=${ClientRole.Devtools}&appClientType=${encodeURIComponent(JSON.stringify(appClientType))}`;
    return {
      id: targetId || clientId,
      clientId,
      thumbnailUrl: '',
      description: 'Hippy instance',
      devtoolsFrontendUrl: `http://localhost:${port}/front_end/inspector.html?remoteFrontend=true&experiments=true&ws=${encodeURIComponent(wsUrl)}`,
      devtoolsFrontendUrlCompat: `http://localhost:${port}/front_end/inspector.html?remoteFrontend=true&experiments=true&ws=${encodeURIComponent(wsUrl)}`,
