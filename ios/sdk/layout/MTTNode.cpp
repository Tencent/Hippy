...
void MTTNode::printNode(uint32_t indent) {
    ...
    MTTLogd(startStr.c_str(), toString(result.dim[0]).c_str(), toString(result.dim[1]).c_str(), toString(result.position[0]).c_str(),
...
bool MTTNode::removeChild(MTTNodeRef child) {
    vector<MTTNodeRef>::iterator p = std::find(children.begin(), children.end(), child);
    if (p != children.end()) {
        children.erase(p);
...
#ifdef LAYOUT_TIME_ANALYZE
    MTTLog(LogLevelDebug, "HippyLayoutTime layout: count %d cache %d, measure: count %d cache %d", layoutCount, layoutCacheCount, measureCount,
        measureCacheCount);
#endif
}
...
void MTTNode::calculateItemsFlexBasis(MTTSize availableSize) {
    ...
    for (size_t i = 0; i < items.size(); i++) {
        ...
        if (isDefined(item->style.getFlexBasis()) && isDefined(style.dim[axisDim[mainAxis]])) {
            ...
            item->layoutImpl(
                availableSize.width, availableSize.height, isRowDirection(mainAxis) ? LayoutActionMeasureWidth : LayoutActionMeasureHeight);
        ...
        item->result.hypotheticalMainAxisMarginBoxSize = item->result.hypotheticalMainAxisSize + item->getMargin(mainAxis);
    }
}
...
bool MTTNode::collectFlexLines(vector<FlexLine *> &flexLines, MTTSize availableSize) {
    ...
    float availableWidth = axisDim[style.flexDirection] == DimWidth ? availableSize.width : availableSize.height;
    ...
    while (i < itemsSize) {
        MTTNodeRef item = items[i];
        if (item->style.positionType == PositionTypeAbsolute || item->style.displayType == DisplayTypeNone) {
        ...
        float leftSpace = availableWidth - (line->sumHypotheticalMainSize + item->result.hypotheticalMainAxisMarginBoxSize);
        if (leftSpace < 0) {
...
void MTTNode::cacheLayoutOrMeasureResult(MTTSize availableSize, MTTSizeMode measureMode, FlexLayoutAction layoutAction) {
...
void MTTNode::layoutSingleNode(
    ...
    if (widthMeasureMode == MeasureModeExactly && heightMeasureMode == MeasureModeExactly) {
        ...
        if (style.flexGrow > 0 && style.flexShrink > 0 && parent && parent->childCount() == 1 && !parent->style.isDimensionAuto(FLexDirectionRow)
        ...
        result.dim[DimWidth]
            = boundAxis(FLexDirectionRow, widthMeasureMode == MeasureModeExactly ? (availableWidth + getPaddingAndBorder(FLexDirectionRow))
                                                                                 : (dim.width + getPaddingAndBorder(FLexDirectionRow)));

        result.dim[DimHeight]
            = boundAxis(FLexDirectionColumn, heightMeasureMode == MeasureModeExactly ? (availableHeight + getPaddingAndBorder(FLexDirectionColumn))
...
void MTTNode::layoutImpl(float parentWidth, float parentHeight, FlexLayoutAction layoutAction) {
    ...
    float nodeWidth = isDefined(style.dim[DimWidth]) ? boundAxis(FLexDirectionRow, style.dim[DimWidth]) : VALUE_UNDEFINED;

    float nodeHeight = isDefined(style.dim[DimHeight]) ? boundAxis(FLexDirectionColumn, style.dim[DimHeight]) : VALUE_UNDEFINED;
    ...
    if (isDefined(style.dim[DimHeight])) {
        ...
        if (parent && parent->style.isOverflowScroll() && isColumnDirection(parent->style.flexDirection)) {
    ...
    MTTSize availableSize = { availableWidth, availableHeight };
    MTTSizeMode measureMode = { widthMeasureMode, heightMeasureMode };
    MeasureResult *cacheResult = layoutCache.getCachedMeasureResult(availableSize, measureMode, layoutAction, measure != nullptr);
    ...
    if ((children.size() == 0)) {
        layoutSingleNode(availableWidth, widthMeasureMode, availableHeight, heightMeasureMode, layoutAction);
        return;
    }
    // 3.Determine the flex base size and hypothetical main size of each item
    ...
    if (isDefined(style.dim[axisDim[mainAxis]])) {
        ...
        if (sumHypotheticalMainSizeOverflow) {  // MeasureModeAtMost
            ...
            float mainInnerSize = axisDim[mainAxis] == DimWidth ? availableSize.width : availableSize.height;

            if (maxSumItemsMainSize > mainInnerSize && !style.isOverflowScroll()) {
                if (parent && parent->getNodeAlign(this) == FlexAlignStretch && axisDim[mainAxis] == axisDim[parent->getCrossAxis()]
...
float MTTNode::determineCrossAxisSize(vector<FlexLine *> &flexLines, MTTSize availableSize, FlexLayoutAction layoutAction) {
    ...
    for (size_t i = 0; i < flexLines.size(); i++) {
        ...
        for (size_t j = 0; j < line->items.size(); j++) {
            ...
            if (getNodeAlign(item) == FlexAlignStretch && item->style.isDimensionAuto(crossAxis) && !item->style.hasAutoMargin(crossAxis)
    ...
    for (size_t i = 0; i < flexLines.size(); i++) {
        ...
        for (size_t j = 0; j < line->items.size(); j++) {
            ...
            if (getNodeAlign(item) == FlexAlignStretch && item->style.isDimensionAuto(crossAxis) && !item->style.hasAutoMargin(crossAxis)) {
                item->result.dim[axisDim[crossAxis]] = item->boundAxis(crossAxis, line->lineCrossSize - item->getMargin(crossAxis));
...
void MTTNode::crossAxisAlignment(vector<FlexLine *> &flexLines) {
    ...
    for (int i = 0; i < linesCount; i++) {
        ...
        for (size_t j = 0; j < line->items.size(); j++) {
            ...
            float remainingFreeSpace = line->lineCrossSize - item->result.dim[axisDim[crossAxis]] - item->getMargin(crossAxis);
            ...
            remainingFreeSpace = line->lineCrossSize - item->result.dim[axisDim[crossAxis]] - item->getLayoutMargin(crossAxis);
    ...
    for (int i = 0; i < linesCount; i++) {
        ...
        for (size_t j = 0; j < line->items.size(); j++) {
            ...
            item->setLayoutStartPosition(crossAxis, crossAxisPostionStart + item->getLayoutStartPosition(crossAxis));
            // layout start position has use relative ,so end position not use it ,use false parameter.
            item->setLayoutEndPosition(
                crossAxis, (getLayoutDim(crossAxis) - item->getLayoutStartPosition(crossAxis) - item->getLayoutDim(crossAxis)), false);
        }
...
void MTTNode::layoutFixedItems(__unused MTTSizeMode measureMode) {
    ...
    for (size_t i = 0; i < items.size(); i++) {
        ...
        if (isUndefined(itemOldStyleDimWidth) && isDefined(item->style.position[CSSLeft]) && isDefined(item->style.position[CSSRight])) {
            item->style.setDim(DimWidth, (getLayoutDim(FLexDirectionRow) - style.border[CSSLeft] - style.border[CSSRight]
                                             - item->style.position[CSSLeft] - item->style.position[CSSRight] - item->getMargin(FLexDirectionRow)));
        }
        if (isUndefined(itemOldStyleDimHeight) && isDefined(item->style.position[CSSTop]) && isDefined(item->style.position[CSSBottom])) {
            item->style.setDim(
                DimHeight, (getLayoutDim(FLexDirectionColumn) - style.border[CSSTop] - style.border[CSSBottom] - item->style.position[CSSTop]
                               - item->style.position[CSSBottom] - item->getMargin(FLexDirectionColumn)));
        }
        ...
        if (isDefined(item->style.position[axisStart[mainAxis]])) {
            item->setLayoutStartPosition(
                mainAxis, getStartBorder(mainAxis) + item->getLayoutStartMargin(mainAxis) + item->style.position[axisStart[mainAxis]]);
            item->setLayoutEndPosition(mainAxis, getLayoutDim(mainAxis) - item->getLayoutStartPosition(mainAxis) - item->getLayoutDim(mainAxis));

        } else if (isDefined(item->style.position[axisEnd[mainAxis]])) {
            item->setLayoutEndPosition(
                mainAxis, getEndBorder(mainAxis) + item->getLayoutEndMargin(mainAxis) + item->style.position[axisEnd[mainAxis]]);
            item->setLayoutStartPosition(mainAxis, getLayoutDim(mainAxis) - item->getLayoutEndPosition(mainAxis) - item->getLayoutDim(mainAxis));

        } else {
            float remainingFreeSpace = getLayoutDim(mainAxis) - getPaddingAndBorder(mainAxis) - item->getLayoutDim(mainAxis);
            ...
            item->setLayoutStartPosition(mainAxis, getStartPaddingAndBorder(mainAxis) + item->getLayoutStartMargin(mainAxis) + offset);
            item->setLayoutEndPosition(mainAxis, getLayoutDim(mainAxis) - item->getLayoutStartPosition(mainAxis) - item->getLayoutDim(mainAxis));
        }
        ...
        if (isDefined(item->style.position[axisStart[crossAxis]])) {
            item->setLayoutStartPosition(
                crossAxis, getStartBorder(crossAxis) + item->getLayoutStartMargin(crossAxis) + item->style.position[axisStart[crossAxis]]);
            item->setLayoutEndPosition(crossAxis, getLayoutDim(crossAxis) - item->getLayoutStartPosition(crossAxis) - item->getLayoutDim(crossAxis));

        } else if (isDefined(item->style.position[axisEnd[crossAxis]])) {
            item->setLayoutEndPosition(
                crossAxis, getEndBorder(crossAxis) + item->getLayoutEndMargin(crossAxis) + item->style.position[axisEnd[crossAxis]]);
            item->setLayoutStartPosition(crossAxis, getLayoutDim(crossAxis) - item->getLayoutEndPosition(crossAxis) - item->getLayoutDim(crossAxis));
        } else {
            float remainingFreeSpace = getLayoutDim(crossAxis) - getPaddingAndBorder(crossAxis) - item->getLayoutDim(crossAxis);
            float offset = getStartPaddingAndBorder(crossAxis);
            ...
            item->setLayoutStartPosition(crossAxis, getStartPaddingAndBorder(crossAxis) + item->getLayoutStartMargin(crossAxis) + offset);
            item->setLayoutEndPosition(crossAxis, getLayoutDim(crossAxis) - item->getLayoutStartPosition(crossAxis) - item->getLayoutDim(crossAxis));
        }
    }
}
...
void MTTNode::convertLayoutResult(float absLeft, float absTop) {
    ...
    const bool hasFractionalWidth = !FloatIsEqual(fmodf(width, 1.0), 0) && !FloatIsEqual(fmodf(width, 1.0), 1.0);
    const bool hasFractionalHeight = !FloatIsEqual(fmodf(height, 1.0), 0) && !FloatIsEqual(fmodf(height, 1.0), 1.0);

    const float absRight = absLeft + width;
    const float absBottom = absTop + height;
    result.dim[DimWidth] = MTTRoundValueToPixelGrid(absRight, (isTextNode && hasFractionalWidth), (isTextNode && !hasFractionalWidth))
                           - MTTRoundValueToPixelGrid(absLeft, false, isTextNode);

    result.dim[DimHeight] = MTTRoundValueToPixelGrid(absBottom, (isTextNode && hasFractionalHeight), (isTextNode && !hasFractionalHeight))
