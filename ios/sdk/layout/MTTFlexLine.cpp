...
void FlexLine::FreezeInflexibleItems(FlexLayoutAction layoutAction) {
	...
	for (size_t i=0; i < items.size(); i++) {
		...
		if (flexFactor == 0 ||
			(flexSign == PositiveFlexibility && item->result.flexBaseSize > item->result.hypotheticalMainAxisSize)||
...
//													offset - item->getLayoutDim(mainAxis);
		...
		item->setLayoutEndPosition(mainAxis, flexContainer->getLayoutDim(mainAxis) -
