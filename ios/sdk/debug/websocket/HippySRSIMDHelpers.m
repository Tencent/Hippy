...
static void HippySRMaskBytesManual(uint8_t *bytes, size_t length, uint8_t *maskKey) {
 ...
 */
static uint8x32_t HippySRShiftVector(uint8x32_t vector, size_t by) {
...
void HippySRMaskBytesSIMD(uint8_t *bytes, size_t length, uint8_t *maskKey) {
    ...
    if (alignmentBytes > length || (length - alignmentBytes) < sizeof(uint8x32_t)) {
        HippySRMaskBytesManual(bytes, length, maskKey);
        return;
    }
    ...
    maskVector = HippySRShiftVector(maskVector, alignmentBytes);

    HippySRMaskBytesManual(bytes, alignmentBytes, maskKey);
    ...
    HippySRMaskBytesManual(bytes + manualStartOffset, manualLength, (uint8_t *) &maskVector);
}
