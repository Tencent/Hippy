...
`(target: 'window' | 'screen') => { height: number, width: number, scale: number, statusBarHeight, navigatorBarHeight, safeArea  }` Hippy Root View 尺寸或者屏幕尺寸。
...
> - safeArea: { left: number, right: number, top: number, bottom: number}; 最低支持版本 2.14.2
