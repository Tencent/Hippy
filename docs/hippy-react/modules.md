...
`(target: 'window' | 'screen') => { height: number, width: number, scale: number, statusBarHeight, navigatorBarHeight  }` 
Hippy Root View 尺寸或者屏幕尺寸。
...
> - target: 'safeArea': 用于获取屏幕的安全区域坐标; 最低支持版本 2.14.2
