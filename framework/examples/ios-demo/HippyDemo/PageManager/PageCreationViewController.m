...
static NSString *const kJSEngineTypeJSC = @"JSC";
static NSString *const kJSEngineTypeHermes = @"Hermes";

static NSString *const kCancel = @"取消";


typedef NS_ENUM(NSUInteger, PageCreationCellIndex) {
    PageCreationCellIndexDriver = 0,
    PageCreationCellIndexRenderer,
    PageCreationCellIndexJSEngine,
    PageCreationCellIndexDebugMode,
    PageCreationCellIndexCount,
};

@interface PageCreationViewController ()<UITableViewDelegate, UITableViewDataSource>

/// TableView for config
@property (nonatomic, strong) UITableView *tableView;

/// Creation button
@property (nonatomic, strong) UIButton *creationButton;

/// Current using dirver
@property (nonatomic, assign) DriverType currentDriver;

/// Current using renderer
@property (nonatomic, strong) NSString *renderer;

/// Current using js engine
@property (nonatomic, strong) NSString *jsEngine;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (PageCreationCellIndexDebugMode == [indexPath section]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (PageCreationCellIndexDriver == [indexPath section]) {
        ...
        return cell;
    } else if (PageCreationCellIndexRenderer == [indexPath section]) {
        PageCreationCell *cell =
            (PageCreationCell *)[tableView dequeueReusableCellWithIdentifier:kNormalCell
                                                                forIndexPath:indexPath];
        cell.summaryImageView.image = [UIImage imageFromIconName:@"render_icon"];
        cell.typeLabel.text = @"Renderer";
        cell.subTypeLabel.text = _renderer;
        return cell;
    }  else if (PageCreationCellIndexJSEngine == [indexPath section]) {
        PageCreationCell *cell =
        (PageCreationCell *)[tableView dequeueReusableCellWithIdentifier:kNormalCell
                                                            forIndexPath:indexPath];
        cell.summaryImageView.image = [UIImage imageFromIconName:@"driver_icon"];
        cell.typeLabel.text = @"JS Engine";
        cell.subTypeLabel.text = _jsEngine;
        return cell;
    } else if (PageCreationCellIndexDebugMode == [indexPath section]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch ([indexPath section]) {
        case PageCreationCellIndexDriver:
            [self showDriverSelectMenu];
            break;
        case PageCreationCellIndexRenderer:
            [self showRenderSelectMenu];
            break;
        case PageCreationCellIndexJSEngine:
            [self showJSEngineSelectMenu];
            break;
        default:
            break;
    }
...
- (void)showJSEngineSelectMenu {
    __weak typeof(self) weakSelf = self;
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction *jscAction = [UIAlertAction actionWithTitle:kJSEngineTypeJSC style:UIAlertActionStyleDefault 
                                                      handler:^(UIAlertAction * _Nonnull action) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (strongSelf) {
            strongSelf.jsEngine = kJSEngineTypeJSC;
            [strongSelf.tableView reloadSections:[NSIndexSet indexSetWithIndex:PageCreationCellIndexJSEngine]
                                withRowAnimation:UITableViewRowAnimationAutomatic];
        }
    }];
    UIAlertAction *hermesAction = [UIAlertAction actionWithTitle:kJSEngineTypeHermes style:UIAlertActionStyleDefault 
                                                         handler:^(UIAlertAction * _Nonnull action) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (strongSelf) {
            strongSelf.jsEngine = kJSEngineTypeHermes;
            [strongSelf.tableView reloadSections:[NSIndexSet indexSetWithIndex:PageCreationCellIndexJSEngine]
                                withRowAnimation:UITableViewRowAnimationAutomatic];
        }
    }];
    [alert addAction:[UIAlertAction actionWithTitle:kCancel style:UIAlertActionStyleCancel handler:nil]];
    [alert addAction:jscAction];
    [alert addAction:hermesAction];
    [self presentViewController:alert animated:YES completion:NULL];
}
...
- (void)createDemoAction {
    ...
    NSURL *debugURL = nil;
    if (_debugMode) {
        DebugCell *cell2 = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:PageCreationCellIndexDebugMode]];
    ...
    HippyDemoViewController *vc = [[HippyDemoViewController alloc] initWithDriverType:driverType 
                                                                      ...
                                                                      useHermesEngine:[_jsEngine isEqualToString:kJSEngineTypeHermes]
                                                                             debugURL:debugURL
