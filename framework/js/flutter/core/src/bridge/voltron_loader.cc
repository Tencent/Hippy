...
#include "bridge/voltron_loader.h"
...
VoltronLoader::VoltronLoader() {}

bool VoltronLoader::RequestUntrustedContent(const unicode_string_view& uri, std::function<void(u8string)> cb) {
  Url parse_url(uri);
  if (!uri_obj) {
...
bool VoltronLoader::RequestUntrustedContent(const unicode_string_view& uri, u8string& content) {
  ...
  if (schema_str == u"file") {
    ...
    std::function<void(u8string)> cb =
        hippy::base::MakeCopyable([p = std::move(promise)](u8string bytes) mutable { p.set_value(std::move(bytes)); });
...
bool VoltronLoader::LoadByFile(const unicode_string_view& path, const std::function<void(u8string)>& cb) {
...
bool VoltronLoader::LoadByAsset(const unicode_string_view& path, const std::function<void(u8string)>& cb,
                            bool is_auto_fill) {
...
bool VoltronLoader::LoadByHttp(const unicode_string_view& uri, const std::function<void(u8string)>& cb) {
  ...
  if (instance->GetMethods().j_fetch_resource_method_id) {
    ...
    j_env->CallVoidMethod(bridge_->GetObj(), instance->GetMethods().j_fetch_resource_method_id, j_relative_path, id);
...
void OnResourceReady(JNIEnv* j_env, __unused jobject j_object, jobject j_byte_buffer, jlong j_runtime_id,
                     jlong j_request_id) {
...
std::function<void(u8string)> VoltronLoader::GetRequestCB(int64_t request_id) {
  auto it = request_map_.find(request_id);
  return it != request_map_.end() ? it->second : nullptr;
}

int64_t VoltronLoader::SetRequestCB(const std::function<void(u8string)>& cb) {
