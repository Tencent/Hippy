...
void callDartMethod(hippy::napi::CBDataTuple* data) {
  ...
  if (info.Length() >= 1 && !info[0].IsEmpty()) {
    ...
    if (module_maybe_str.IsEmpty()) {
      isolate->ThrowException(v8::Exception::TypeError(
          v8::String::NewFromOneByte(isolate, reinterpret_cast<const uint8_t*>("module name error")).ToLocalChecked()));
      return;
    }
    unicode_string_view module_name_view = v8_ctx->ToStringView(module_maybe_str.ToLocalChecked());
    module_name = StrViewToCU16String(module_name_view);
    TDF_BASE_DLOG(INFO) << "callDartMethod module_name = " << module_name_view;
  } else {
    isolate->ThrowException(v8::Exception::Error(
        v8::String::NewFromOneByte(isolate, reinterpret_cast<const uint8_t*>("info error")).ToLocalChecked()));
    return;
  }
  ...
  if (info.Length() >= 2 && !info[1].IsEmpty()) {
    ...
    if (func_maybe_str.IsEmpty()) {
      isolate->ThrowException(v8::Exception::TypeError(
          v8::String::NewFromOneByte(isolate, reinterpret_cast<const uint8_t*>("func name error")).ToLocalChecked()));
      return;
    }
    unicode_string_view module_func_view = v8_ctx->ToStringView(func_maybe_str.ToLocalChecked());
  ...
  if (info.Length() >= 4 && !info[3].IsEmpty() && info[3]->IsObject()) {
    if (runtime->IsEnableV8Serialization()) {
      ...
      std::shared_ptr<hippy::napi::V8CtxValue> obj = std::make_shared<hippy::napi::V8CtxValue>(isolate, info[3]);
  ...
  runtime->GetPlatformRuntime()->CallNaive(module_name, module_func, call_id, buffer_data, buffer_length,
                                           !runtime->IsEnableV8Serialization(), nullptr, true);
}
}  // namespace bridge
