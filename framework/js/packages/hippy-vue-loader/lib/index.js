...
function loadTemplateCompiler(loaderContext) {
  try {
    ...
    if (/version mismatch/.test(e.toString())) {
      ...
      loaderContext.emitError(new Error('[vue-loader] vue-template-compiler must be installed as a peer dependency, '
...
module.exports = function (source) {
  ...
  // styles
  let stylesCode = '';
  let styleRequestList = [];
  if (descriptor.styles.length) {
    ({ stylesCode, styleRequestList } = genStylesCode(
      loaderContext,
      descriptor.styles,
      id,
      resourcePath,
...
var component = normalizer(
  ...
  if (needsHotReload) {
    code += `\n${genHotReloadCode(id, hasFunctional, templateRequest, styleRequestList)}`;
  }
