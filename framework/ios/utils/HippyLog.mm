...
void HippyAddLogFunction(HippyLogFunction logFunction) {
    ...
    if (existing) {
        HippySetLogFunction(^(HippyLogLevel level, NSString *fileName, NSNumber *lineNumber, NSString *message, NSArray<NSDictionary *> *stack, __weak HippyBridge *bridge) {
            existing(level, fileName, lineNumber, message, stack, bridge);
            logFunction(level, fileName, lineNumber, message, stack, bridge);
        });
    } else {
...
void HippyPerformBlockWithLogPrefix(void (^block)(void), NSString *prefix) {
    ...
    if (logFunction) {
        HippyPerformBlockWithLogFunction(
            block, ^(HippyLogLevel level, NSString *fileName, NSNumber *lineNumber, NSString *message, NSArray<NSDictionary *> *stack, __weak HippyBridge *bridge) {
                logFunction(level, fileName, lineNumber, [prefix stringByAppendingString:message], stack, bridge);
            });
    }
}
...
void HippyLogNativeInternal(HippyLogLevel level, __weak HippyBridge *bridge, const char *fileName, int lineNumber, NSString *format, ...) {
    ...
    if (log && level >= HippyGetLogThreshold()) {
        ...
        // Call log function
        if (logFunction) {
            logFunction(level, fileName ? @(fileName) : nil, lineNumber > 0 ? @(lineNumber) : nil, message, callStacks, bridge);
        }
    }
}
