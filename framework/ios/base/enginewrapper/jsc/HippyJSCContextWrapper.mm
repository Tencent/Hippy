    ...
    if (self) {
        ...
        if (strongContext) {
            ...
            _context.exceptionHandler = ^(JSContext *context, JSValue *exception) {
                @autoreleasepool {
                    HippyJSCContextWrapper *strongSelf = weakSelf;
                    if (!strongSelf) {
                        return;
                    }
                    ExceptionHandler exceptionHandler = strongSelf.excpetionHandler;
                    if (!exceptionHandler) {
                        return;
                    }
                    JSGlobalContextRef contextRef = strongSelf->_context.JSGlobalContextRef;
                    NSString *stacksString = [[exception objectForKeyedSubscript:@"stack"] toString];
                    NSString *message = [[exception objectForKeyedSubscript:@"message"] toString];
...
id<HippyContextWrapper> CreateContextWrapper(std::shared_ptr<hippy::napi::Ctx> context) {
    return [[HippyJSCContextWrapper alloc] initWithContext:context];
}
