...
@implementation HPConvert (Deprecated)
...
+ (UIImage *)UIImage:(id)json {
    ...
    if (!HPIsMainQueue()) {
        ...
        HPLogWarn(@"Calling [HPConvert UIImage:] on a background thread is not recommended");
    ...
    NSURL *URL = HPURLWithString(uri, NULL);
    NSString *scheme = URL.scheme.lowercaseString;
    if ([scheme isEqualToString:@"file"]) {
        NSString *assetName = HPBundlePathForURL(URL);
        ...
        if (!image) {
            ...
            if (!image) {
                HPLogConvertError(json, @"an image. File not found.");
            }
        }
    } else if ([scheme isEqualToString:@"data"]) {
        image = [UIImage imageWithData:[NSData dataWithContentsOfURL:URL]];
    } else {
        HPLogConvertError(json, @"an image. Only base64 image data are supported.");
        return nil;
    }
    return image;
}
