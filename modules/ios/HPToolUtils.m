...
NSString *const HPErrorUnspecified = @"HPErrorUnspecified";
...
NSString *__nullable HPJSONStringify(id __nullable jsonObject, NSError **error) {
    if (error) {
        ...
        if (localError) {
            ...
            return _HPJSONStringifyNoRetry(HPJSONClean(jsonObject), NULL);
        }
        return json;
    }
}
...
id __nullable HPJSONParse(NSString *__nullable jsonString, NSError **error) {
    return _HPJSONParse(jsonString, NO, error);
}

id __nullable HPJSONParseMutable(NSString *__nullable jsonString, NSError **error) {
    return _HPJSONParse(jsonString, YES, error);
}

id HPJSONClean(id object) {
    static dispatch_once_t onceToken;
...
CGFloat HPRoundPixelValue(CGFloat value) {
    CGFloat scale = HPScreenScale();
    return round(value * scale) / scale;
}

CGFloat HPCeilPixelValue(CGFloat value) {
    CGFloat scale = HPScreenScale();
    return ceil(value * scale) / scale;
}

CGFloat HPFloorPixelValue(CGFloat value) {
    CGFloat scale = HPScreenScale();
    return floor(value * scale) / scale;
}

CGSize HPSizeInPixels(CGSize pointSize, CGFloat scale) {
    return (CGSize) {
...
NSDictionary<NSString *, id> *HPMakeError(NSString *message, id __nullable toStringify, NSDictionary<NSString *, id> *__nullable extraData) {
    if (toStringify) {
...
NSDictionary<NSString *, id> *HPMakeAndLogError(NSString *message, id __nullable toStringify, NSDictionary<NSString *, id> *__nullable extraData) {
    NSDictionary<NSString *, id> *error = HPMakeError(message, toStringify, extraData);
    HPLogError(@"\nError: %@", error);
    return error;
}

NSDictionary<NSString *, id> *HPJSErrorFromNSError(NSError *error) {
    NSString *codeWithDomain = [NSString stringWithFormat:@"E%@%ld", error.domain.uppercaseString, (long)error.code];
    return HPJSErrorFromCodeMessageAndNSError(codeWithDomain, error.localizedDescription, error);
}

NSDictionary<NSString *, id> *HPJSErrorFromCodeMessageAndNSError(NSString *code, NSString *message, NSError *__nullable error) {
...
NSURL *HPDataURL(NSString *mimeType, NSData *data) {
...
NSString *__nullable HPBundlePathForURL(NSURL *__nullable URL) {
    if (!URL.fileURL) {
        // Not a file path
        return nil;
    }
...
BOOL HPIsLocalAssetURL(NSURL *__nullable imageURL) {
    NSString *name = HPBundlePathForURL(imageURL);
    if (!name) {
        return NO;
    }
...
NSString *__nullable HPTempFilePath(NSString *extension, NSError **error) {
    static NSError *setupError = nil;
    static NSString *directory;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        directory = [NSTemporaryDirectory() stringByAppendingPathComponent:@"HPNative"];
        ...
        if (![fileManager fileExistsAtPath:directory]) {
            ...
            if (![fileManager createDirectoryAtPath:directory withIntermediateDirectories:YES attributes:nil error:&localError]) {
                // This is bad
                HPLogError(@"Failed to create temporary directory: %@", localError);
