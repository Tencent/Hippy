...
const mockDataArray = [
  ...
  {
    title: '[广告]--李宁，一切皆有可能！',
    coverUrl: 'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Ftr-osdcp.qunarzz.com%2Ftr-osd-tr-manager%2Fimg%2Ffdaaa35607bef4e427c24f21863d33af.jpg&refer=http%3A%2F%2Ftr-osdcp.qunarzz.com&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1621233279&t=d76a8216302e491453213a6d4ea31cf2',
    templateType: 1,
    type: 6,
  },
  {
    title: '《Lose Yourself》',
    coverUrl: 'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fy.gtimg.cn%2Fmusic%2Fphoto_new%2FT015R640x360M1010033fX2B0wL8wW.jpg&refer=http%3A%2F%2Fy.gtimg.cn&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1621259106&t=547cfd12d432d66bc0607475dc9fd087',
    author: 'Eminem',
    likeNum: 1001,
    commentsNum: 232,
    type: 1,
...
export default class FeedsExpo extends React.Component {
  ...
  getRenderRow(index) {
    ...
    const isLastItem = this.state.dataSource.length === index + 1;
    ...
    if (rowData.type === STYLE_LOADING) {
      ...
      if (this.wormholeEnabled(index)) {
        // 通过params传递虫洞数据
        rowView = <TKDWormhole ref={ref => this.wormholes[index] = ref}
                               style={{ width: 0, height: 0 }}
                               params={{ data: rowData, index }}>
  ...
  onCellClick(index, title) {
    if (index % 2 === 0) {
      callNative('EventObserver', 'postWormholeMessage', {
        fromModule: 'Feeds', toModule: 'wormhole', rootTag: this.props.rootTag, eventName: 'feedsCardDidClick',
        extData: { index, title },
      });
    } else {
  ...
  refreshData = async () => {
    ...
    const { fetchingDataFlag } = this.state;
    if (fetchingDataFlag) return;
    const tmpLoading = [{ type: STYLE_LOADING }];
    this.setState({
      fetchingDataFlag: true,
