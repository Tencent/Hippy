...
void UIManagerModule::CallUIFunction(CallbackInfo& info, void* data) {
  ...
  if (flag) {
    ...
    cb = [weak_scope, weak_function, id](const std::shared_ptr<DomArgument> &argument) -> void {
      auto scope = weak_scope.lock();
      if (!scope) {
        return;
      }
      auto cb = [weak_scope, weak_function, argument, id]() {
        auto scope  = weak_scope.lock();
        if (!scope) {
          return;
        }
        ...
        scope->removeCallUIFunctionCallback(id);
      };
      auto runner = scope->GetTaskRunner();
      FOOTSTONE_CHECK(runner);
      runner->PostTask(std::move(cb));
    };
    scope->AddCallUIFunctionCallback(id, function);
  }
