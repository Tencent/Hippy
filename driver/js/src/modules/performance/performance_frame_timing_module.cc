...
std::shared_ptr<ClassTemplate<PerformanceFrameTiming>> RegisterPerformanceFrameTiming(const std::weak_ptr<Scope>& weak_scope) {
  ClassTemplate<PerformanceFrameTiming> class_template;
  class_template.name = "PerformanceFrameTiming";
  class_template.constructor = [weak_scope](
      const std::shared_ptr<CtxValue>& receiver,
      size_t argument_count,
      const std::shared_ptr<CtxValue> arguments[],
      void* external,
      std::shared_ptr<CtxValue>& exception) -> std::shared_ptr<PerformanceFrameTiming> {
