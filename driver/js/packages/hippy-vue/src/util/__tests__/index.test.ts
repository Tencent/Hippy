...
test('trace output test', (t) => {
  ...
  util.trace('Hello world', { a: 1 });
  t.pass();
  (Vue.config as any).silent = false;
  process.env.NODE_ENV = 'production';
  util.trace('Hello world', { a: 1 });
  t.pass();
  delete process.env.NODE_ENV;
  // @ts-expect-error TS(2540): Cannot assign to 'release' because it is a read-on... Remove this comment to see the full error message
...
test('arrayCount test', (t) => {
  const arr = new Array(10).fill(0)
    .map((a, index) => index);
  t.is(util.arrayCount(arr, (a: any) => a === 1), 1);
  t.is(util.arrayCount(arr, (a: any) => a < 5), 5);
});
