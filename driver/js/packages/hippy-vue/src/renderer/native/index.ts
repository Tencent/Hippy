...
if (typeof global.Symbol !== 'function') {
  // @ts-expect-error TS(2322): Type 'string | number | undefined' is not assignab... Remove this comment to see the full error message
 ...
 */
function chunkNodes(batchNodes: any) {
  const result = [];
...
function handleEventListeners(eventNodes = [], sceneBuilder: any) {
  eventNodes.forEach((eventNode) => {
    if (eventNode) {
      const { id, eventList } = eventNode;
      (eventList as any).forEach((eventAttribute: any) => {
 ...
 */
function printNodeOperation(printedNodes: any, nodeType: any) {
  if (isTraceEnabled()) {
    trace(...LOG_TYPE, nodeType, printedNodes);
  }
}

function endBatch(app: any) {
  if (!batchIdle) return;
  batchIdle = false;
...
function getCssMap() {
  ...
  if (global[GLOBAL_DISPOSE_STYLE_NAME]) {
    // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
    global[GLOBAL_DISPOSE_STYLE_NAME].forEach((id: any) => {
      cssMap.delete(id);
    });
    // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
 ...
 */
function getNativeProps(node: any) {
 ...
 * @param style
 */
function parseTextInputComponent(targetNode: any, style: any) {
 ...
 * @param style
 */
function parseViewComponent(targetNode: any, nativeNode: any, style: any) {
 ...
 * @returns {{}}
 */
function getElemCss(element: any) {
  const style = Object.create(null);
  try {
    getCssMap().query(element).selectors.forEach((matchedSelector: any) => {
 ...
 * @returns attributes|{}
 */
function getTargetNodeAttributes(targetNode: any) {
  try {
...
function processModalView(nativeNode: any) {
 ...
 * @param targetNode
 */
function getEventNode(targetNode: any) {
 ...
 * @param targetNode
 */
function getNativeNode(rootViewId: any, targetNode: any, refInfo = {}, style: any) {
  const nativeNode = {
    id: targetNode.nodeId,
 ...
 */
function renderToNative(rootViewId: any, targetNode: any, refInfo = {}) {
 ...
 * @returns {[]}
 */
function renderToNativeWithChildren(rootViewId: any, node: any, callback: any, refInfo = {}) {
  const nativeLanguages: any = [];
  const eventLanguages: any = [];
  const printedLanguages: any = [];
  node.traverseChildren((targetNode: any, refInfo: any) => {
...
function isLayout(node: any, rootView: any) {
...
function insertChild(parentNode: any, childNode: any, refInfo = {}) {
...
function removeChild(parentNode: any, childNode: any) {
...
function moveChild(parentNode: any, childNode: any, refInfo = {}) {
...
function updateEvent(parentNode: any) {
...
function updateChild(parentNode: any, notUpdateStyle = false) {
  if (!parentNode.isMounted) {
    return;
  }
  const app = getApp();
...
function updateWithChildren(parentNode: any) {
  if (!parentNode.isMounted) {
    return;
  }
  const app = getApp();
