...
test('findChild test', (t) => {
  ...
  const childNode = new ViewNode();
  const childNode2 = new ViewNode();
  (childNode2 as any).metaTest = 123;
  parentNode.appendChild(childNode);
  childNode.appendChild(childNode2);
  const targetNode = parentNode.findChild((node: any) => node.metaTest === 123);
  t.is(targetNode, childNode2);
  const targetNode2 = parentNode.findChild((node: any) => node.metaTest === 234);
...
test('traverseChildren test', (t) => {
  ...
  parentNode.traverseChildren((node: any) => {
